...
const Body = () => {
  ...
  const [states, useStates] = useState(0);
  useEffect(() => {
    data();
  }, []);
  const data = async () => {
    const reslists = await fetch(
      "https://www.swiggy.com/dapi/restaurants/list/v5?lat=17.4117312&lng=78.3424898&is-seo-homepage-enabled=true&page_type=DESKTOP_WEB_LISTING"
    );
    const rests = await reslists.json();
    setRest(
      rests.data.cards[4].card.card.gridElements.infoWithStyle.restaurants
    );
    setfiltered(
      rests.data.cards[4].card.card.gridElements.infoWithStyle.restaurants
    );
  ...
  const filter = () => {
    setfiltered(rest.filter((item) => item.info.avgRating > 4));
  };
  const filters = () => {
    setfiltered(rest.filter((item) => item.info.sla.deliveryTime < 25));
  };
  const filterss = () => {
    setfiltered(rest.filter((item) => item.info.sla.lastMileTravel < 2));
  };
  const veg = () => {
    setfiltered(rest.filter((item) => item.info.veg == true));
  };
  ...
  return (
    <div className="contains">
                  ...
                  <div className="container-fluid bar-s">
                    <form className="d-flex search-bar" role="search">
                      ...
                      <button
                        ...
                        type="submit"
                        onClick={() => {
                          const filtersearch = rest.filter((res) => {
                            res.info.name.includes(searchs);
                          });
                          setRest(filtersearch);
                        }}
                      >
                        Search
                      </button>
                ...
                <li className="nav-item">
                  <a className="nav-link" href="#">
                    <span className="cart_count">
                      ...
                      <span className="cart_count_value"></span>
                    </span>
                    <span>Cart</span>
                  </a>
                </li>
      ...
      <h1 className="filter-text">Filter Restaurants by 👌</h1>
      <div className="buttons">
        <button onClick={filter} className="btn btn-outline-secondary">
          top hotels
        </button>
        <button onClick={filters} className="btn btn-outline-secondary">
          Delivery time
        </button>
        <button onClick={veg} className="btn btn-outline-secondary">
          pure veg
        </button>
        <button onClick={filterss} className="btn btn-outline-secondary">
          near me
        </button>
        <button
          onClick={() => setfiltered(rest)}
      ...
      <div className="contains1">
        {filtered.map((items) => (
          <Cards
            addeditems={items.info.name}
            name={items.info.name}
            rating={items.info.avgRating}
            des={items.info.costForTwo}
            img_id={items.info.cloudinaryImageId}
            del_time={items.info.sla.deliveryTime}
            hello={() => {
              useStates(states + 1);
            }}
          />
